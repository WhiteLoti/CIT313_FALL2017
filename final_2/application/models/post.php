...
class Post extends Model{
	...
	public function getAllPosts($limit = 0){
	    ...
	    $sql =  "SELECT t1.pID, t1.title, t1.content, t1.date, t1.uID, t1.categoryID,
                    t2.uID, t2.first_name, t2.last_name,
                    t3.categoryID, t3.name FROM posts t1 INNER JOIN users t2 INNER JOIN categories t3 ON 
                    t1.uID = t2.uID AND t1.categoryID = t3.categoryID AND t3.categoryID = ".$cID;
            ...
            $sql = "SELECT name FROM categories WHERE categoryID =".$cID;

            $result = $this->db->execute($sql);
            $result = (array)$result;

            array_unshift($posts,array('name'=>$result['fields']['name']));
