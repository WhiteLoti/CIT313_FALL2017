...
<?php
  ...
  {
     echo '<div class="alert alert-warning">Awaiting Activation Approval</div>';
  }
?>
  <div id="app">
    <form class="api" id="weather_app" action="<?php echo BASE_URL.'weather/weather' ?>" method="POST">
      <div class="close" title="Close weather app"> [x] </div><br><br>
      Enter An Address: <input type="text" name="zip">
      <input class="btn btn-secondary" required="true" type="submit" value="Get Weather">
      <div class="result">
      </div>
    </form>
  </div>
  <div>
      <?php 
         ...
         {
             echo '<img id="profile_picture" class="image" src="'.BASE_URL.'views/img/profile_pictures/'.$profile_picture.'">';
         }
         else
         {
             echo '<img id="profile_picture" class="image" src="'.BASE_URL.'views/img/profile_pictures/blank.png">';
         }
         echo '</div>';
      ?>
    ...
    <td colspan="4">
      ...
      <div class="info">
        <?php
          ...
          if($school != '')
          {
            $edutation .= 'School: '.$school.'<br>';
          }
          if($major != '')
          {
            $edutation .= 'Major: '.$major.'<br>';
          }
          if($concentration != '')
          {
            $edutation .= 'Concentration: '.$concentration.'<br>';
          }
          if($graduation != '')
          {
            $edutation .= 'Expected Graduation: '.$graduation.'<br>';
          }
          if($education === '')
          {
            echo 'The user chooses not to display their education information';
          }
        ?>
      </div>
